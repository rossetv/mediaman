...
from mediaman.services.storage import delete_path  # re-exported for back-compat
...
class ScanEngine:
    ...
    def __init__(
        ...
        *,
        ...
        self._arr_cache = ArrDateCache(
            radarr_client=radarr_client, sonarr_client=sonarr_client,
        )
    ...
    def _build_arr_date_cache(self) -> None:  # pragma: no cover — compat shim
        ...
        self._arr_cache._loaded = False  # noqa: SLF001 — intentional for shim
        self._arr_cache.ensure_loaded()

    @property
    def _arr_dates(self) -> dict[str, str]:
        return self._arr_cache._dates  # noqa: SLF001 — compat read-through
    ...
    def sync_library(self) -> dict[str, int]:
        ...
        for lib_id in self._library_ids:
            try:
                buffered.extend(self._fetcher.fetch_library_items(lib_id))
                try:
    ...
    def _fetch_library_items(self, library_id: str) -> list[_PlexItemFetch]:
        """Back-compat shim — delegates to :class:`PlexFetcher`."""
        return self._fetcher.fetch_library_items(library_id)
    ...
    def run_scan(self) -> dict[str, int]:
        ...
        """
        summary = {
            "scanned": 0, "scheduled": 0, "skipped": 0, "errors": 0, "removed": 0,
        }
        ...
        # Guard against library IDs that cannot be coerced to int
        # (malformed settings would raise ValueError here without the
        # try/except, and the scan result would be lost).
        ...
        if _get_bool_setting(self._conn, "suggestions_enabled", default=True):
            try:
                ...
                logger.exception(
                    "Recommendation generation failed — scan results unaffected"
                )
    ...
    def _resolve_added_at(self, item: dict) -> datetime:
        ...
        back to the DB ``updated_at`` / ``added_at`` fields when no Arr
        record exists. Falls back to ``datetime.now(UTC)`` via
        ``ensure_tz(None)`` when both are ``None``.
        """
        arr_date_str = self._arr_cache.get(item.get("file_path", ""))
        if arr_date_str:
            return _ensure_tz(
                _parse_iso_utc(arr_date_str) or datetime.now(timezone.utc)
            )
    ...
    def _scan_movie_library(
        ...
        def _evaluate(
            f: _PlexItemFetch, added_at: Any, watch_history: list,
        ) -> str | None:
    ...
    def _remove_orphaned_items(
        ...
        * If the scan found less than ``_MIN_RATIO_TO_TRUST`` (10 %) of
        ...
        previous_count = repository.count_items_in_libraries(
            self._conn, list(scanned_libs)
        )
        ...
        if (
            current_count < _MIN_ITEMS_TO_TRUST
            and previous_count >= _MIN_ITEMS_TO_TRUST
        ):
            logger.warning(
        ...
        all_ids = repository.fetch_ids_in_libraries(
            self._conn, list(scanned_libs)
        )
    ...
    # ------------------------------------------------------------------
    # Thin wrappers over repository helpers — preserved so tests that
    # monkeypatch these method names keep working.
    # ------------------------------------------------------------------
    ...
    def _is_protected(self, media_id: str) -> bool:
        return repository.is_protected(self._conn, media_id)

    def _is_already_scheduled(self, media_id: str) -> bool:
        return repository.is_already_scheduled(self._conn, media_id)

    def _has_expired_snooze(self, media_id: str) -> bool:
        return repository.has_expired_snooze(self._conn, media_id)

    def _is_show_kept(self, show_rating_key: str | None) -> bool:
        return repository.is_show_kept(self._conn, show_rating_key)

    def _update_last_watched(
        self, media_id: str, watch_history: list[dict],
    ) -> None:
