...
def update_last_watched(
    conn: sqlite3.Connection, media_id: str, watch_history: list[dict]
) -> None:
...
def delete_scheduled_action(conn: sqlite3.Connection, action_id: int) -> None:
    ...
    conn.execute(
        "DELETE FROM scheduled_actions WHERE id = ?", (action_id,)
    )
