...
def _load_or_create_salt(conn: sqlite3.Connection) -> bytes:
    ...
    Raises:
        RuntimeError: If the stored salt is corrupt (cannot be base64-decoded),
            ...
            or if the salt inserted on first run has an unexpected length after
            the INSERT/re-read cycle.
    """
...
def decrypt_value(
    ...
    *,
    ...
    For v2 ciphertexts (prefixed ``0x02``), decryption is attempted first with
    the supplied *aad* and then, if that raises ``InvalidTag``, once more
    without AAD to handle values encrypted before AAD binding was introduced.
    If both attempts fail, the legacy v1 path is tried silently and a debug
    log entry is emitted.

    For v1 ciphertexts (no prefix byte), the legacy SHA-256-derived key is
    used directly and a ``WARNING`` is logged prompting the operator to
    re-encrypt the value.

    Raises:
        ValueError: If *encrypted* is empty or exceeds the maximum ciphertext
            length.
        InvalidTag: When a v2 ciphertext fails authentication and no v1
            fallback is possible (i.e. the raw bytes carry the v2 prefix but
            authentication failed with every key variant, or the raw bytes are
            too short to be a valid v1 ciphertext).
    """
    if not encrypted:
