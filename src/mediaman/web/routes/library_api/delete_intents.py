...
def reconcile_pending_delete_intents() -> int:
    ...
    for row in pending:
        ...
        except Exception as exc:  # rationale: §6.4 site 4 — cold-start recovery; one bad intent must not stop the rest of the reconciliation loop.
            with contextlib.suppress(sqlite3.Error):
                conn.execute("ROLLBACK")
            _fail_delete_intent(conn, intent_id, str(exc))
            logger.exception(
                "delete_intent.reconcile_failed intent_id=%s media_id=%s",
                intent_id,
                media_item_id,
            )
