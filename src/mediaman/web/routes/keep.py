...
def keep_submit(request: Request, token: str, duration: str = Form(default="")) -> Response:
    ...
    if duration == "forever":
        ...
        new_execute = now + timedelta(days=days)  # type: ignore[arg-type]  # VALID_KEEP_DURATIONS values are int but typed as object because the dict also contains "forever"; the isinstance guard above ensures days is int here
        cursor = conn.execute(
            "UPDATE scheduled_actions SET action=?, token_used=1, "
