...
SQL against the ``settings`` table lives in
:mod:`mediaman.web.repository.settings` — the routes never touch the
table directly (§2.7.1) and decryption happens inside the repository
(§9.9). The legacy ``_load_settings`` name is re-exported below for
back-compatibility with external callers that import it directly.
"""
...
from mediaman.web.repository.settings import (
    fetch_encrypted_key_set as _fetch_encrypted_key_set,
)
from mediaman.web.repository.settings import (
    load_settings as _load_settings,  # back-compat re-export for tests
)
from mediaman.web.repository.settings import (
...
def settings_page(request: Request) -> Response:
    ...
    enc_keys = _fetch_encrypted_key_set(conn)
...
def api_update_settings(
    ...
    :func:`mediaman.web.repository.settings.write_settings` so we never
    have a "settings changed but no audit trail" outcome for
    high-impact mutations (M27).
    """
