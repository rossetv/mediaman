...
class ArrUpstreamError(ArrError):
    """Upstream Sonarr/Radarr returned a malformed or unusable response.

    Used for situations the protocol declares well-formed (200 with a JSON
    body) but which violate the shape the caller depends on — for example,
    a null body, an array where a dict was expected, or a missing required
    field.  Distinct from :class:`ArrConfigError` (which indicates the
    operator misconfigured the service) and from
    :class:`~mediaman.services.infra.http.client.SafeHTTPError` (which
    indicates a non-2xx status).
    """
...
class _ArrClientBase:
    ...
    def _get(self, path: str) -> dict | list:
        ...
        Raises:
            ArrUpstreamError: If the response body is null (empty or explicitly null JSON).
            SafeHTTPError: For non-2xx responses or transport-layer failures
                that the SSRF-aware client surfaces.
            requests.RequestException: For other ``requests``-layer failures
                (e.g. JSON decode errors are :class:`requests.exceptions.JSONDecodeError`).
        """
        try:
            ...
            return result
        except (SafeHTTPError, RequestException, ArrUpstreamError) as exc:
    ...
    def _put(self, path: str, data: dict) -> None:
        ...
        try:
            ...
            self.last_error = None
        except (SafeHTTPError, RequestException) as exc:
            self.last_error = str(exc)
            raise
    ...
    def _post(self, path: str, data: dict) -> dict | list:
        ...
        Raises:
            SafeHTTPError: For non-2xx responses or transport-layer failures.
            requests.RequestException: For other ``requests``-layer failures
                including JSON decode of the response body.
        """
        try:
