"""Shared HTTP plumbing and spec-driven unified client for *arr-family APIs.

The module exposes a single concrete :class:`ArrClient` that talks to either
Sonarr (``kind="series"``) or Radarr (``kind="movie"``) depending on the
:class:`~mediaman.services.arr.spec.ArrSpec` passed to its constructor.  All
service-specific methods live on this class; kind-specific ones raise
:exc:`ArrKindMismatch` when called on the wrong variant.
...
:attr:`ArrClient.last_error` is ``None`` when the last call succeeded and is
set to the exception string on failure so UI layers can display a banner
...
class ArrClient:
    ...
    #: Class-level defaults for the per-instance add-flow caches; populated
    #: lazily by :meth:`_choose_root_folder` / :meth:`_choose_quality_profile`
    #: on first use and then read from the instance.
    ...
    def _unmonitor_with_retry(
        ...
        *,
        ...
        Both :meth:`unmonitor_season` and :meth:`unmonitor_movie` implement
