...
from mediaman.services.recommendations.enrich import enrich_recommendations
from mediaman.services.recommendations.prompts import generate_personal, generate_trending
...
def refresh_recommendations(
    ...
    trending = generate_trending(conn, previous_titles)
    personal = generate_personal(conn, watch_history, user_ratings, previous_titles) if watch_history else []
    all_recommendations = trending + personal

    if not all_recommendations:
        return 0

    enrich_recommendations(all_recommendations, conn)
