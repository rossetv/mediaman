...
class ArrClient:
    ...
    def lookup_by_tmdb_id(self, tmdb_id: int, *, endpoint: str) -> list[dict]:
        ...
        """
        return self._get(f"{endpoint}?term=tmdb:{tmdb_id}") or []  # type: ignore[return-value]

    def lookup_by_tvdb_id(self, tvdb_id: int, *, endpoint: str) -> list[dict]:
        ...
        """
        return self._get(f"{endpoint}?term=tvdb:{tvdb_id}") or []  # type: ignore[return-value]

    def lookup_by_imdb_id(self, imdb_id: str, *, endpoint: str) -> list[dict]:
        ...
        """
        return self._get(f"{endpoint}?term=imdb:{imdb_id}") or []  # type: ignore[return-value]

    def lookup_by_term(self, term: str, *, endpoint: str) -> list[dict]:
        """Return lookup results for a free-text search term.

        ``term`` must already be URL-encoded by the caller if it contains
        spaces or special characters.
        """
        return self._get(f"{endpoint}?term={term}") or []  # type: ignore[return-value]
