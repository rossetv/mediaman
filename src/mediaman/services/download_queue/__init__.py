...
from mediaman.services.arr_search_trigger import maybe_trigger_search
from mediaman.services.download_queue._deep_links import (
...
from mediaman.services.download_queue._items import (
    build_episode_dicts,
)
from mediaman.services.download_queue._items import (
...
from mediaman.services.download_queue._nzb_match import nzb_matches_arr as _nzb_matches_arr_impl
from mediaman.services.download_queue._response import DownloadsResponse
...
# ---------------------------------------------------------------------------
# Re-export the private name used by tests
# ---------------------------------------------------------------------------

# Tests access ``_nzb_matches_arr`` directly via the module namespace.
def _nzb_matches_arr(nzb_t_norm: str, arr_candidates: list[str]) -> bool:
    """Proxy for the module-level function — preserves the private name for tests."""
    return _nzb_matches_arr_impl(nzb_t_norm, arr_candidates)
...
def _build_unmatched_arr_item(arr: dict, arr_base_urls_map: dict[str, str]) -> dict:
...
def build_downloads_response(conn: sqlite3.Connection) -> DownloadsResponse:
    ...
    arr_base_urls_map = _arr_base_urls(conn)
    ...
    from mediaman.services.arr_build import build_arr_client as _build_arr_client_local

    active_ids = {item["id"] for item in items}
    active_titles = {item["title"] for item in items}
    recent = fetch_and_sync_recent_downloads(
        conn, active_ids, active_titles, _build_arr_client_local,
    )
