...
| `MEDIAMAN_ALLOWED_HOSTS` | no | (any) | Comma-separated list of allowed `Host:` header values (e.g. `mediaman.example.com`). Unset means all hosts are accepted (Starlette `AllowedHostsMiddleware` defaults to `["*"]`). Always set this in production. Loopback (`localhost`, `127.0.0.1`) is appended automatically so the in-container healthcheck survives a pinned allowlist — no need to list it yourself. |
