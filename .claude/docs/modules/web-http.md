...
| `src/mediaman/web/routes/library_api/` | Keep/delete/redownload JSON API. `__init__.py` composes `_redownload_router` + `_delete_router`; `_redownload_match.py`/`_redownload_radarr.py`/`_redownload_sonarr.py` resolve the Arr record to re-grab. |
