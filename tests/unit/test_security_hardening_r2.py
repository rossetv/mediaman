...
class TestCsrfExemptPrefix:
    def test_unsubscribe_exact_match(self):
        ...
        assert c.post(
            "/unsubscribe", headers={"Origin": "https://mail.com"}
        ).status_code == 200
        # /unsubscribe-admin must NOT inherit the exemption.
        assert c.post(
            "/unsubscribe-admin", headers={"Origin": "https://evil.com"}
        ).status_code == 403
    ...
    def test_unsubscribe_slash_exempt(self):
        ...
        c = TestClient(app)
        assert c.post(
            "/unsubscribe/confirm", headers={"Origin": "https://mail.com"}
        ).status_code == 200
...
class TestSessionFingerprint:
    ...
    def test_fingerprint_rejects_different_client(self, tmp_path):
        ...
        assert validate_session(
            conn, token, user_agent="Mozilla/5.0 X", client_ip="192.0.2.99",
        ) == "alice"

        # Different UA — cookie theft → reject.
        assert validate_session(
            conn, token, user_agent="Chrome attacker", client_ip="192.0.2.1",
        ) is None
    ...
    def test_unbound_session_works_without_fingerprint(self, tmp_path):
        ...
        assert validate_session(
            conn, token, user_agent="anything", client_ip="1.2.3.4",
        ) == "bob"
...
class TestSessionIdleTimeout:
    def test_idle_session_expires(self, tmp_path):
        ...
        conn.execute(
            "UPDATE admin_sessions SET last_used_at = ?", (past,)
        )
        conn.commit()
