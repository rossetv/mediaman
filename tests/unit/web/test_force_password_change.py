...
class TestForceChangePost:
    ...
    def test_policy_violation_renders_issues(self, conn):
        app = _make_app(conn)
        with patch(SESSION_PATCH, return_value=("admin", conn)):
            with patch("mediaman.web.routes.force_password_change.password_issues", return_value=["Too short"]):
    ...
    def test_wrong_old_password_logs_security_event(self, conn):
        app = _make_app(conn)
        with patch(SESSION_PATCH, return_value=("admin", conn)):
            with patch("mediaman.web.routes.force_password_change.password_issues", return_value=[]):
                with patch("mediaman.web.routes.force_password_change.change_password", return_value=False):
                    with patch("mediaman.web.routes.force_password_change.security_event") as mock_event:
                        resp = self._post(app, old_password="wrong", new_password="Str0ng!Pass", confirm_password="Str0ng!Pass")
    ...
    def test_success_rotates_session_and_redirects(self, conn):
        ...
        with patch(SESSION_PATCH, return_value=("admin", conn)):
            with patch("mediaman.web.routes.force_password_change.password_issues", return_value=[]):
                with patch("mediaman.web.routes.force_password_change.change_password", return_value=True):
                    with patch("mediaman.auth.session.create_session", return_value="new-token"):
                        client = TestClient(app, raise_server_exceptions=True)
                        resp = client.post(
                            "/force-password-change",
                            data={"old_password": "old", "new_password": "Strong!Pass1", "confirm_password": "Strong!Pass1"},
