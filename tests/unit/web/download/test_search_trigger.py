...
class TestSearchTriggerThrottle:
    ...
    def test_stale_released_movie_triggers_search(self, monkeypatch, freezer):
        ...
        item = {
            ...
            "arr_id": 42,
            "is_upcoming": False,
            "added_at": time.time() - 600,  # 10 minutes ago (relative to frozen clock)
        }
        maybe_trigger_search(conn, item, matched_nzb=False, secret_key="test-key")

        mock_radarr.search_movie.assert_called_once_with(42)

    def test_second_call_within_two_minutes_does_not_trigger(self, monkeypatch, freezer):
    ...
    def test_upcoming_item_does_not_trigger_search(self, monkeypatch, freezer):
        """An upcoming item must never trigger a search regardless of staleness."""
        import time
    ...
    def test_recently_added_item_does_not_trigger_search(self, monkeypatch, freezer):
        """An item added within the 5-min staleness window must not trigger a search."""
        import time
    ...
    def test_matched_nzb_item_does_not_trigger_search(self, monkeypatch, freezer):
        """An item already matched to an NZB download must not trigger an additional search."""
        import time
    ...
    def test_series_triggers_search_series(self, monkeypatch, freezer):
        """A stale TV series item must trigger a Sonarr series search."""
        import time
    ...
    def test_trigger_after_16_min_fires_again(self, monkeypatch, freezer):
        ...
        maybe_trigger_search(conn, item, matched_nzb=False, secret_key="test-key")
        # Rewind the stored timestamp by 16 minutes (relative to frozen clock)
