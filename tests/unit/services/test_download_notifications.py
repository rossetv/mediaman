...
class TestSonarrSeriesMatching:
    ...
    def _setup(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "mediaman.services.mailgun.MailgunClient",
    ...
    def test_tvdb_only_series_matches(self, tmp_path, monkeypatch):
        ...
        from mediaman.services.download_notifications import (
        ...
        monkeypatch.setattr(
            "mediaman.services.arr_build.build_sonarr_from_db",
            lambda *a, **kw: sonarr_client,
        )
        monkeypatch.setattr(
            "mediaman.services.arr_build.build_radarr_from_db",
            lambda *a, **kw: None,
        )
        ...
        row = conn.execute(
            "SELECT notified FROM download_notifications"
        ).fetchone()
    ...
    def test_v11_migration_moves_sonarr_ids_to_tvdb_column(self, tmp_path):
        ...
        rows = conn.execute(
            "SELECT title, tmdb_id, tvdb_id, service "
            "FROM download_notifications ORDER BY title"
        ).fetchall()
