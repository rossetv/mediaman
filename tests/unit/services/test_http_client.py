...
class TestSizeCap:
    def test_refuses_declared_oversize_body(self, monkeypatch):
        ...
        resp = _response(
            status=200, body=b"", headers={"Content-Length": too_big},
        )
        monkeypatch.setattr(
            http_client, "_dispatch",
            lambda *a, **kw: resp,
        )
    ...
    def test_refuses_streamed_oversize_body(self, monkeypatch):
        ...
        with pytest.raises(SafeHTTPError):
            SafeHTTPClient(default_max_bytes=4 * 1024 * 1024).get(
                "http://example.com/"
            )
...
class TestSSRFReCheck:
    def test_refuses_when_guard_rejects(self, monkeypatch):
        ...
        monkeypatch.setattr(
            http_client, "is_safe_outbound_url",
